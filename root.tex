...
velocity, and a short-window odometry cue. We deploy \SAS{} on a four-track robot and evaluate it against manual
...
traversal quality under both the ground-truth map and the onboard map, while using
neither. The biggest difference is on obstacles unobservable to the robot's sensors, such
as a rigid log buried in tall grass, where map-based control breaks down while \SAS{} keeps
the robot stabilized and operational. \TODO{final quantitative headline once results are in.}
\end{abstract}
