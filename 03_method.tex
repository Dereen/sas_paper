...
velocity per flipper. Two terms are formed independently: a stabilization term that drives
the body back towards level, and an anti-stuck term that digs the flippers down when the
robot is commanded forward but is not advancing. A directional-priority rule combines them,
so the anti-stuck term can reinforce the stabilization term but never reverse it, and the
result is clamped to the flipper speed and position limits. The rest of this section
defines each term in turn.
...
Eq.~\eqref{eq:overview}, a directional \emph{clamp}: when $v^{\mathrm{d}}_i$ opposes
$v^{\mathrm{s}}_i$ it clamps the anti-stuck action into the band
$[-\alpha|v^{\mathrm{s}}_i|,\,\alpha|v^{\mathrm{s}}_i|]$ set by a fraction $\alpha\in[0,1]$ of the
stabilization demand, but only when stabilization meaningfully acts ($|v^{\mathrm{s}}_i|$
above a small dead-band $\beta$, so noise on a level robot cannot veto a dig); otherwise it
passes $v^{\mathrm{d}}_i$ through:
\begin{equation}
  \mathrm{cap}(v^{\mathrm{d}}_i,v^{\mathrm{s}}_i)=
  \begin{cases}
    \mathrm{clamp}\!\big(v^{\mathrm{d}}_i,\,-\alpha|v^{\mathrm{s}}_i|,\,\alpha|v^{\mathrm{s}}_i|\big)\\
    \quad\text{if } v^{\mathrm{s}}_i v^{\mathrm{d}}_i<0 \ \wedge\ |v^{\mathrm{s}}_i|>\beta,\\[2pt]
    v^{\mathrm{d}}_i \quad\text{otherwise.}
  \end{cases}
  \label{eq:cap}
\end{equation}
